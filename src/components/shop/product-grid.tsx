...
import { ChevronLeft, ChevronRight, Package, ChevronRight as ArrowRight } from 'lucide-react';
...
export function ProductGrid({ filters, className }: ProductGridProps) {
  ...
  const [pagination, setPagination] = useState({
    page: 1,
    limit: PAGINATION.MAX_LIMIT, // เปลี่ยนเป็นแสดง 100 รายการ
    total: 0,
    totalPages: 0,
  });
  ...
  return (
    <div className={className}>
      {/* Products Grid Grouped by Game */}
      <div className="space-y-8 mb-8">
        {Object.entries(
          products.reduce((acc, product) => {
            const gameName = product.game?.name || 'อื่นๆ';
            if (!acc[gameName]) {
              acc[gameName] = [];
            }
            acc[gameName].push(product);
            return acc;
          }, {} as Record<string, ProductWithGame[]>)
        ).map(([gameName, gameProducts]) => {
          // ถ้ามีเกมที่เลือกและไม่ตรงกับเกมปัจจุบัน ให้ข้าม
          ...
          return (
            <div key={gameName} className="space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-bold text-gradient">{gameName}</h2>
                {!selectedGame ? (
                  <Button
                    variant="ghost"
